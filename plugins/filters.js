...
const afnp = Badboy.PLKAFN !== false ? Badboy.PLKAFN.split(',') : [];
...
MyPnk.addCommand({on: 'text', fromMe: false }, (async (message, match) => {
    if(Config.BGMFILTER){
        ...
        if (!!message.mention && message.mention[0] == '916282185054@s.whatsapp.net') {
...
if(pattern.test(message.message)){
       await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/uploads/' + a + '.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio,duration: Config.SAID, quoted: message.data, ptt: true})
}
});
    }
...
MyPnk.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
    ...
    let banned = jid.find( Jid => Jid === message.jid);
    if(banned !== undefined) return
    if (!!message.mention && message.mention[0] == '994404407243@s.whatsapp.net') {
